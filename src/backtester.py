...
def run_backtest(backtester: BacktestEngine) -> PerformanceMetrics | None:
    ...
    try:
        ...
        print(f"\n{Fore.GREEN}{t('backtest_completed')}{Style.RESET_ALL}")
        return performance_metrics
    except KeyboardInterrupt:
        print(f"\n\n{Fore.YELLOW}{t('backtest_interrupted')}{Style.RESET_ALL}")
        
        # Try to show any partial results that were computed
        try:
            portfolio_values = backtester.get_portfolio_values()
            if len(portfolio_values) > 1:
                print(f"{Fore.GREEN}{t('partial_results')}{Style.RESET_ALL}")
                ...
                print(f"{Fore.CYAN}{t('initial_portfolio_value')}: ${first_value:,.2f}{Style.RESET_ALL}")
                print(f"{Fore.CYAN}{t('final_portfolio_value')}: ${last_value:,.2f}{Style.RESET_ALL}")
                print(f"{Fore.CYAN}{t('total_return')}: {total_return:+.2f}%{Style.RESET_ALL}")
        except Exception as e:
            print(f"{Fore.RED}{t('could_not_generate_partial')}: {str(e)}{Style.RESET_ALL}")
